...
impl AnthropicClient {
    ...
    async fn send_raw_request(
        ...
        apply_prompt_cache_breakpoints(&mut request_body);
...
/// Rewrite the outgoing request body so Anthropic's prompt-caching kicks in
/// on the two longest stable prefixes: the system prompt and the tools
/// list. The Messages API recognises `cache_control: {"type":"ephemeral"}`
/// at the **last** breakpoint of each prefix (max 4 breakpoints total) and
/// turns subsequent identical prefixes into a 50–90 % cheaper cache read.
///
/// We only mark a system prompt for caching if it's long enough to pay for
/// itself — the minimum cacheable prefix is ~1024 tokens (≈4 KB ascii) for
/// the small/sonnet tier and 2048 for haiku. Using char length as a cheap
/// proxy: skip anything under 3 KB so we don't waste a breakpoint on a
/// trivial prompt.
///
/// Non-Anthropic providers ignore unknown fields — but this function only
/// runs on the Anthropic code path so that's not a worry here.
fn apply_prompt_cache_breakpoints(body: &mut Value) {
    const MIN_SYSTEM_CHARS_TO_CACHE: usize = 3000;

    let Some(object) = body.as_object_mut() else {
        return;
    };

    // System prompt: convert string → array form with cache_control on the
    // single text block. The Messages API accepts either shape; the array
    // form is the only one that supports cache_control.
    if let Some(sys) = object.get("system").cloned() {
        if let Some(text) = sys.as_str() {
            if text.len() >= MIN_SYSTEM_CHARS_TO_CACHE {
                object.insert(
                    "system".to_string(),
                    serde_json::json!([{
                        "type": "text",
    ...
    // Tools: stamp cache_control on the last tool. This caches the entire
    // tools array as a single prefix — and our tools list is stable across
    // a session (built from `mvp_tool_specs()` + MCP discovery once).
...
mod tests {
    ...
    #[test]
    fn cache_breakpoint_added_to_long_system_prompt() {
        let mut body = serde_json::json!({
        ...
        super::apply_prompt_cache_breakpoints(&mut body);
        let sys = body.get("system").and_then(serde_json::Value::as_array)
            .expect("system should be array form after caching");
        assert_eq!(sys.len(), 1);
        assert_eq!(
            sys[0].get("cache_control")
                .and_then(|v| v.get("type"))
                .and_then(serde_json::Value::as_str),
            Some("ephemeral")
        );
        let tools = body.get("tools").and_then(serde_json::Value::as_array).unwrap();
    ...
    #[test]
    fn cache_breakpoint_skipped_for_short_system_prompt() {
        let mut body = serde_json::json!({ "system": "short" });
        super::apply_prompt_cache_breakpoints(&mut body);
        // Stays as a plain string when below the cache threshold.
