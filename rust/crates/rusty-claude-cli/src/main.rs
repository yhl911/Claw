...
use tools::{
    agent_store_dir_pub, execute_tool, mvp_tool_specs, GlobalToolRegistry, RuntimeToolDefinition,
    ToolSearchOutput,
};
...
- `opc-finance`     — 财务分析、成本核算、商业建模\n\
- `opc-marketing`   — 市场推广、文案撰写、渠道策略\n\
- `opc-ops`         — 运营规划、流程设计、项目管理\n\
\n\
### 工作流程\n\
1. 使用 Agent 工具（subagent_type 设为上述角色之一）委派子任务，可同时发起多个并发执行\n\
2. 使用 read_file 读取返回的 manifest_file 路径，检查 JSON 中的 \"status\" 字段\n\
   - \"running\" → 等待，稍后再查\n\
   - \"completed\" → 使用 read_file 读取 output_file 获取结果\n\
   - \"failed\" → 查看 \"error\" 字段，决定是否重试\n\
3. 所有任务完成后，聚合输出，向用户呈现完整结果\n\
\n\
### 原则\n\
- 优先将独立任务并行委派以提高效率\n\
- 每次委派前在 description 中明确任务目标和交付标准\n\
- 用中文与用户沟通，除非用户明确要求其他语言\n\
- 你是决策者，sub-agent 是执行者；最终输出需经过你的审阅和整合\
";
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Opc {
            ...
            allowed_tools,
            permission_mode,
        } => run_opc_repl(model, allowed_tools, permission_mode)?,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    if rest.is_empty() {
        ...
        if args.contains(&String::from("--opc")) {
            return Ok(CliAction::Opc {
                model,
                allowed_tools,
                permission_mode,
            });
...
fn run_resume_command(
    ...
    match command {
        ...
        | SlashCommand::AddDir { .. }
        | SlashCommand::OpcAgents => Err("unsupported resumed slash command".into()),
    }
}
...
fn opc_startup_banner(model: &str) -> String {
    format!(
        "\x1b[38;5;220m\
 ██████╗ ██████╗  ██████╗\n\
██╔═══██╗██╔══██╗██╔════╝\n\
...
██║   ██║██╔═══╝ ██║     \n\
╚██████╔╝██║     ╚██████╗\n\
 ╚═════╝ ╚═╝      ╚═════╝\x1b[0m \x1b[38;5;208mCEO Mode\x1b[0m 🏢\n\n\
  \x1b[2mModel\x1b[0m    {model}\n\
  \x1b[2mRole\x1b[0m     CEO Agent (OPC)\n\
\n\
...
fn run_opc_repl(
    ...
    let resolved_model = resolve_repl_model(model);
    let mut cli = LiveCli::new(resolved_model, true, allowed_tools, permission_mode)?;
    cli.system_prompt.push(OPC_CEO_SYSTEM_PROMPT.to_string());

    let mut editor =
        input::LineEditor::new("CEO> ", cli.repl_completion_candidates().unwrap_or_default());
    println!("{}", opc_startup_banner(&cli.model));
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::OpcAgents => {
                Self::print_opc_agents();
                false
            }
    ...
    fn print_opc_agents() {
        let store_dir = match tools::agent_store_dir_pub() {
            Ok(dir) => dir,
            Err(_) => {
                println!("No OPC agents found (agent store not initialized).");
                return;
            }
        };
        ...
        for entry in entries.flatten() {
            ...
            let subagent_type = v["subagent_type"].as_str().unwrap_or("").to_string();
            ...
            agents.push((agent_id, subagent_type, status, preview));
        }
        if agents.is_empty() {
            println!("No OPC sub-agents in this session. Use the Agent tool with subagent_type='opc-*' to create one.");
            return;
        }
        agents.sort_by(|a, b| a.0.cmp(&b.0));
        println!("\n\x1b[1mOPC Sub-Agents\x1b[0m\n");
        for (id, role, status, preview) in &agents {
            ...
            println!(
                "  \x1b[2m{id}\x1b[0m  \x1b[36m{role}\x1b[0m  {status_color}{status}\x1b[0m\n    {preview}"
            );
        }
        println!();
    }
