...
pub trait ToolExecutor {
    ...
    /// Execute multiple tool calls together. The default implementation runs
    /// them sequentially via `execute`. Implementations whose underlying
    /// dispatch is stateless or thread-safe may override this to parallelize
    /// (e.g. via `std::thread::scope`).
    ///
    /// The runtime calls this once per assistant turn after all permission
    /// checks have passed, with one entry per approved `tool_use` block. The
    /// returned `Vec` MUST have the same length as `calls`, in the same
    /// order, so the caller can pair results with their `tool_use_id`.
    fn execute_batch(
        &mut self,
        calls: &[(String, String)],
...
/// Internal: a tool call that passed the pre-hook + permission check and is
/// ready to be dispatched via `ToolExecutor::execute_batch`.
#[derive(Clone)]
struct ApprovedCall {
    tool_use_id: String,
    tool_name: String,
...
/// Internal: outcome of one pending `tool_use` after permission resolution.
/// Indexed positions in the `approved` vector preserve the original `tool_use`
/// order so post-hook and session push happen in the same order the model
/// emitted them, even if execution was concurrent.
#[derive(Clone)]
enum PendingOutcome {
    /// Permission denied — the result message is already known.
    Denied(ConversationMessage),
    /// Permission allowed — index into `approved` vector.
    Approved(usize),
}
    ...
    T: ToolExecutor,
{
    ...
    pub fn run_turn(
        ...
        loop {
            ...
            // Three-phase processing of pending tool calls so the executor
            // can batch-execute (potentially in parallel) without losing the
            // sequential semantics required by hooks and session updates.
            //
            // Phase 1 — Sequential pre-hook + permission check. We collect
            //   `Approved { tool_use_id, tool_name, input, pre_hook }` for
            //   each call that may run, and immediately materialize a
            //   denied tool_result for ones blocked by hook or policy.
            // Phase 2 — Batch execute the approved calls via the executor's
            //   `execute_batch` (default impl is sequential; desktop runs
            //   them in parallel via `thread::scope`). One outcome per
            //   approved call, in order.
            // Phase 3 — Sequential post-hook + session push, in original
            //   tool_use order across both denied and approved calls.

            let mut approved: Vec<ApprovedCall> = Vec::new();
            let mut outcomes: Vec<PendingOutcome> = Vec::with_capacity(pending_tool_uses.len());

            for (tool_use_id, tool_name, input) in pending_tool_uses.iter().cloned() {
                ...
                match permission_outcome {
                    PermissionOutcome::Allow => {
                        ...
                        outcomes.push(PendingOutcome::Approved(idx));
                    }
                    PermissionOutcome::Deny { reason } => {
                        ...
                        outcomes.push(PendingOutcome::Denied(msg));
                    }
                }
            }

            // Phase 2: batch-execute approved calls.
            let batch_input: Vec<(String, String)> = approved
                .iter()
                .map(|c| (c.tool_name.clone(), c.effective_input.clone()))
                .collect();
            let batch_results = self.tool_executor.execute_batch(&batch_input);
            debug_assert_eq!(batch_results.len(), approved.len());

            // Phase 3: sequential post-hook + session push, in original order.
            for outcome in outcomes {
                let result_message = match outcome {
                    PendingOutcome::Denied(msg) => msg,
                    PendingOutcome::Approved(idx) => {
                        let call = &approved[idx];
                        let exec_result = &batch_results[idx];
                        let (mut output, mut is_error) = match exec_result {
                            Ok(output) => (output.clone(), false),
                            Err(error) => (error.to_string(), true),
                        };
                        output = merge_hook_feedback(call.pre_hook.messages(), output, false);

                        let post_hook_result = if is_error {
                            self.run_post_tool_use_failure_hook(
                                &call.tool_name,
                                &call.effective_input,
                                &output,
                            )
                        } else {
    ...
    /// Mutable access to the tool executor. Used by desktop layer to reset
    /// per-turn state (e.g. loop detector ring buffer) before each `run_turn`.
