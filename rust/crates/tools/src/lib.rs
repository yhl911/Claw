...
fn run_agent(input: AgentInput) -> Result<String, String> {
    // Synchronous execution: block until the sub-agent finishes and return
    // its final output as the tool_result. This is the contract the parent
    // agent sees — no manifest paths, no polling, no "running" status.
    to_pretty_json(execute_agent_sync(input)?)
}
...
/// Default cap on how many iterations a sub-agent's `run_turn` loop may
/// take before bailing with "conversation loop exceeded the maximum number
/// of iterations". The original 32 was too tight for OPC sub-agents that
/// do multi-step research (read several files + web search + structure
/// output). 80 gives generous headroom while still preventing genuine
/// infinite loops.
///
/// Override at runtime via the `CLAWD_AGENT_MAX_ITERATIONS` env var.
const DEFAULT_AGENT_MAX_ITERATIONS: usize = 80;

fn agent_max_iterations() -> usize {
    std::env::var("CLAWD_AGENT_MAX_ITERATIONS")
        .ok()
...
/// Async (fire-and-forget) Agent execution. Currently unused at runtime —
/// the desktop's `Agent` tool routes through [`execute_agent_sync`] for the
/// synchronous semantics described in the OPC architecture doc. Kept here
/// because tests and future use cases (e.g. a separate `AgentAsync` tool
/// for very long-running work) may need to spawn without blocking.
#[allow(dead_code)]
fn execute_agent(input: AgentInput) -> Result<AgentOutput, String> {
    execute_agent_with_spawn(input, spawn_agent_job)
}

/// Result returned to the CEO when the `Agent` tool is invoked synchronously.
/// Deliberately minimal: the CEO does NOT need to know about `manifest_file` /
/// `output_file` paths because the final output is delivered directly. Including
/// those paths previously confused the model into re-spawning Agents to "read"
/// their own output instead of using the already-delivered text.
#[derive(Debug, Clone, Serialize)]
pub struct AgentSyncResult {
    /// Stable identifier — the UI panel still uses this to link to the manifest
    /// file for live observability. CEO can ignore it.
    #[serde(rename = "agentId")]
    pub agent_id: String,
    /// Human-readable name (slug derived from `name` input or description).
    pub name: String,
    /// Canonical role / `subagent_type` after alias normalization.
    #[serde(rename = "subagentType")]
    pub subagent_type: String,
    /// Whether the sub-agent reached a terminal `completed` state.
    pub status: String,
    /// Final assistant text from the sub-agent. This is what the CEO acts on.
    pub output: String,
    /// Populated only when the sub-agent failed.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<String>,
}

/// Execute the `Agent` tool **synchronously**. The current thread blocks until
/// the sub-agent finishes (success, failure, or panic). The returned struct is
/// what becomes the `tool_result` the CEO sees.
///
/// This replaces the old async-with-manifest pattern, which forced the CEO to
/// poll `read_file` to discover when sub-agents finished. Models conflated
/// "read manifest" with "spawn another agent" and looped forever; the sync
/// pattern eliminates that class of bug entirely.
///
/// Manifest + output files on disk are still written so the desktop UI can
/// show real-time agent status, but they are NOT part of the CEO's contract.
fn execute_agent_sync(input: AgentInput) -> Result<AgentSyncResult, String> {
    // Reuse all the validation + manifest-writing code by giving
    // execute_agent_with_spawn a "spawn function" that just records the job.
    let captured: std::cell::RefCell<Option<AgentJob>> = std::cell::RefCell::new(None);
    let manifest = execute_agent_with_spawn(input, |job| {
        *captured.borrow_mut() = Some(job);
        Ok(())
    })?;
    let job = captured
        .into_inner()
        .ok_or_else(|| "internal: agent job was not captured".to_string())?;
    ...
    // Run inline (blocks). Wrapped in catch_unwind so a panic in the
    // sub-agent's runtime doesn't kill the calling worker thread.
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| run_agent_job(&job)));

    match result {
        Ok(Ok(final_text)) => Ok(AgentSyncResult {
            agent_id,
            name,
            subagent_type,
        ...
        Ok(Err(err)) => {
            let _ = persist_agent_terminal_state(&job.manifest, "failed", None, Some(err.clone()));
            Ok(AgentSyncResult {
                agent_id,
                name,
                subagent_type,
        ...
        Err(_) => {
            let panic_msg = String::from("sub-agent panicked during execution");
...
/// Spawn a sub-agent in a background thread. See [`execute_agent`] for why
/// this is currently dead code at runtime.
#[allow(dead_code)]
...
fn run_agent_job(job: &AgentJob) -> Result<String, String> {
    let mut runtime = build_agent_runtime(job)?.with_max_iterations(agent_max_iterations());
    let summary = runtime
...
/// Detailed role prompts adapted from VoltAgent/awesome-claude-code-subagents
/// (MIT). See `crates/tools/prompts/opc-roles/ATTRIBUTION.md` for source mapping.
const OPC_PRODUCT_PROMPT: &str = include_str!("../prompts/opc-roles/opc-product.md");
const OPC_ENGINEERING_PROMPT: &str = include_str!("../prompts/opc-roles/opc-engineering.md");
const OPC_FINANCE_PROMPT: &str = include_str!("../prompts/opc-roles/opc-finance.md");
const OPC_MARKETING_PROMPT: &str = include_str!("../prompts/opc-roles/opc-marketing.md");
const OPC_SALES_PROMPT: &str = include_str!("../prompts/opc-roles/opc-sales.md");
const OPC_OPS_PROMPT: &str = include_str!("../prompts/opc-roles/opc-ops.md");
const OPC_LEGAL_PROMPT: &str = include_str!("../prompts/opc-roles/opc-legal.md");

const OPC_SUBAGENT_PREAMBLE: &str = "\
你是 OPC（一人公司）CEO 委派的专业 sub-agent。\n\n\
\
## ⚠️ 关键约束（必须遵守）\n\n\
\
1. **工具调用预算：最多 10 次**。如果超过 10 次工具调用还没产出最终回答，\
立即基于已有信息给出最终回答，不要再调任何工具。\n\
\
2. **优先用你自己的知识**。除非真的需要（读代码文件、查实时数据、运行命令），\
**否则不要调用任何工具**。直接基于训练知识回答即可。\n\
\
3. **必须以最终回答结束**。你的最后一条消息必须是**纯文本的最终交付物**\
（如分析报告、PRD、邮件、代码方案）—— 不能以工具调用结尾。\n\
\
4. **不要试图执行决策**。即使你的角色描述提到 \"drive decisions\"、\
\"implement\" 等动词，你的产出**只是文档/方案**，不是真的去 write_file 改代码、\
不是真的去发邮件。除非任务明确要求生成代码文件。\n\
\
5. **不要向用户提问**。你看不到用户，CEO 已经把任务转达给你。如果信息不够，\
基于合理假设给出方案，并在方案末尾标注 \"假设：…\"。\n\n\
\
## 工作流程\n\n\
\
1. 读取下方角色描述，理解你的专业领域。\n\
2. 看 CEO 在 user message 里给的具体任务。\n\
3. **快速判断**：\n   \
- 这个任务靠你的训练知识能回答吗？→ 直接写最终回答（**0 次工具调用**）\n   \
- 需要看本地文件/代码？→ 读 1~3 个最相关的文件，然后写最终回答\n   \
- 需要查实时信息？→ WebSearch 1~2 次，然后写最终回答\n\
4. 输出结构化的 markdown 最终回答。包含小标题、要点、关键结论。\n\n\
\
## 你的专业角色描述\n\n";
...
fn build_agent_system_prompt(
    ...
    let role_body: Option<&str> = match subagent_type {
        ...
        "opc-finance" => Some(OPC_FINANCE_PROMPT),
        "opc-marketing" => Some(OPC_MARKETING_PROMPT),
        "opc-sales" => Some(OPC_SALES_PROMPT),
        "opc-ops" => Some(OPC_OPS_PROMPT),
        "opc-legal" => Some(OPC_LEGAL_PROMPT),
        _ => None,
    };
...
fn resolve_agent_model(model: Option<&str>) -> String {
    // Highest priority: explicit `model` field on the Agent tool input.
    if let Some(explicit) = model.map(str::trim).filter(|m| !m.is_empty()) {
        return explicit.to_string();
    }
    // Next: process env override (set by the desktop app from settings).
    // This lets sub-agents inherit the CEO's configured model so they hit the
    // same provider/base URL — critical when running against non-Anthropic
    // backends (DeepSeek, Tencent MaaS, etc.) where `claude-opus-4-6` would
    // route to the wrong endpoint and 404.
    if let Ok(env_model) = std::env::var("CLAWD_DEFAULT_AGENT_MODEL") {
...
#[allow(clippy::too_many_lines)] // long match table is intentional
fn allowed_tools_for_subagent(subagent_type: &str) -> BTreeSet<String> {
    let tools = match subagent_type {
        ...
        // Engineering needs the full developer toolkit (REPL, NotebookEdit, Skill).
        "opc-engineering" => vec![
            "bash",
            "read_file",
            "write_file",
        ...
        // Non-engineering OPC roles share the same conservative toolkit.
        "opc-product"
        | "opc-finance"
        | "opc-marketing"
        | "opc-ops"
...
/// For `DeepSeek` thinking-capable models, force `thinking_mode` to disabled so
/// the sub-agent's tool-calling path stays simple. Returns `None` for models
/// that don't have a `DeepSeek`-style thinking knob.
fn thinking_mode_for_subagent(model: &str) -> Option<bool> {
    let bare = model.strip_prefix("openai/").unwrap_or(model);
    if bare.starts_with("deepseek-v") || bare.starts_with("deepseek-reasoner") {
        Some(false)
    } else {
        None
    }
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        for (index, entry) in chain.iter().enumerate() {
            let message_request = MessageRequest {
                ...
                stream: true,
                // Force thinking-mode OFF for sub-agents on DeepSeek thinking
                // models. Multi-turn + tool_use + reasoning_content passback
                // is fragile and routinely 400s; sub-agents almost never need
                // thinking output anyway.
                thinking_mode: thinking_mode_for_subagent(&entry.model),
                ..Default::default()
            };
...
pub fn agent_store_dir_pub() -> Result<std::path::PathBuf, String> {
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "opcproduct" | "opcprd" => String::from("opc-product"),
        "opcengineering" | "opceng" | "opcdev" => String::from("opc-engineering"),
        "opcfinance" | "opcfin" => String::from("opc-finance"),
        "opcmarketing" | "opcmkt" => String::from("opc-marketing"),
        "opcops" | "opcoperations" => String::from("opc-ops"),
        "opcsales" | "opcsale" | "opcsell" => String::from("opc-sales"),
        "opclegal" | "opclaw" | "opccompliance" => String::from("opc-legal"),
        _ => trimmed.to_string(),
    }
}
